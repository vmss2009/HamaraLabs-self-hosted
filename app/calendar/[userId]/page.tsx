...
function PublicCalendar({ selectedDate, onDateSelect, schedules, userName }: PublicCalendarProps) {
  ...
  return (
    <div className="w-full">
      <div
        className="flex items-center justify-between mb-6 px-2 rounded-lg border p-3"
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <button
              ...
              className="px-3 py-1.5 hover:bg-opacity-20 transition-colors"
            ...
            <button
              onClick={nextMonth}
              className="px-3 py-1.5 hover:bg-opacity-20 transition-colors border-l"
              style={{
                background: "color-mix(in srgb, var(--foreground) 5%, transparent)",
                borderColor: "color-mix(in srgb, var(--foreground) 20%, transparent)",
              }}
            ...
            >
              ...
              {hasSlots && availableCount > 0 && (
                <div className="mt-1 flex flex-col items-center">
                  ...
                  <div className={`text-xs mt-0.5 ${isSelected ? "text-current opacity-80" : "opacity-70"}`}>
                    {availableCount === 1 ? "1 slot" : `${availableCount} slots`}
                  </div>
                </div>
              )}
              {isTodayDate && (
      ...
      <div className="flex flex-wrap gap-4 mt-6 px-2 text-sm">
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 rounded border-2" style={{ borderColor: "var(--foreground)" }} /> <span>Today</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-4 h-4 rounded" style={{ background: "var(--foreground)" }} /> <span>Selected</span>
        </div>
        <div className="flex items-center gap-2">
          <div
...
interface SchedulePageProps { params: Promise<{ userId: string }> }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setUser(userData);
        if (userData && userData.isPublic === false) {
          setError("This user's schedule is currently private.");
          return;
        }

        const schedulesResponse = await fetch(`/api/schedules/public/id/${userId}?leadMinutes=30`);
        if (!schedulesResponse.ok) throw new Error("Failed to fetch schedules");
        const schedulesData = await schedulesResponse.json();
        const formatted = schedulesData
          .map((s: { id: string; userId: string; date: string; timeSlots: TimeSlot[]; createdAt: string; updatedAt: string }) => ({
            ...s,
    ...
    (date: Date) => {
      ...
      if (isBefore(picked, today)) {
        notify({
          ...
          description: "You can only view availability for today and future dates.",
          variant: "destructive",
        });
        return;
      }
  ...
  async function submitBooking() {
    ...
    try {
      ...
      const res = await fetch("/api/bookings", {
        ...
        body: JSON.stringify({ userId, date: format(selectedDate, "yyyy-MM-dd"), startTime: selectedSlot.startTime, endTime: selectedSlot.endTime, guestName, guestEmail, notes }),
      });
      if (!res.ok) {
      ...
      notify({
        title: "Booked!",
        description: `You're in for ${formatTimeTo12Hour(selectedSlot.startTime)}-${formatTimeTo12Hour(selectedSlot.endTime)}.`,
      });
          ...
          >
            ...
            {selectedDate && isBefore(selectedDate, today) ? (
              ...
              <div>
                <div className="mb-4">
                  <p className="text-sm">
                    {selectedSchedule.timeSlots.length} time slot
                    {selectedSchedule.timeSlots.length === 1 ? "" : "s"} available
                  </p>
                </div>
                  ...
                  {selectedSchedule.timeSlots.map((slot) => (
                    <button
                      ...
                      className={`p-3 border rounded-md text-left ${
                        selectedSlot?.id === slot.id ? "ring-2" : ""
                      }`}
                      style={{
                        borderColor: "color-mix(in srgb, var(--foreground) 20%, transparent)",
                      }}
                ...
                <div className="mt-4 grid grid-cols-1 gap-2">
                  ...
                  />
                  <button
                    disabled={!selectedSlot || !guestName || !guestEmail || isBooking}
                    ...
                    style={{
                      ...
                      opacity:
                        !selectedSlot || !guestName || !guestEmail || isBooking
                          ? 0.6
                          : 1,
                    }}
                  >
