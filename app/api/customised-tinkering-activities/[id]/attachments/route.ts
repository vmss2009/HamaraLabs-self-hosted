...
import { createTAAttachments, SnapshotAttachmentInput } from "@/lib/db/snapshot-attachments/crud";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const created = await createTAAttachments(id, attachments);
    return success(created, 201);
  } catch (error) {
    console.error("Error creating snapshot attachments:", error);
